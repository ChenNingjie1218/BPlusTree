...
TEST_F(SEARCH_TREE, output_all_the_keys_test) {
  ...
  ASSERT_EQ(_test_tree->OutPutAllTheKeys(), ans)
...
TEST_F(SEARCH_TREE, serialize_and_deserialize_test) {
  ...
  if (fr) {
    ...
    EXPECT_EQ(_test_tree->BFS(), serializeTree->BFS())
        << "serialize and deserialize error: BFS";
    EXPECT_EQ(_test_tree->OutPutAllTheKeys(), serializeTree->OutPutAllTheKeys())
...
TEST_F(SEARCH_TREE, Concurrent) {
  vector<int> ans = _test_tree->OutPutAllTheKeys();
  ...
  threads.clear();
  EXPECT_EQ(_test_tree->OutPutAllTheKeys(), insertAns)
  ...
  EXPECT_EQ(_test_tree->OutPutAllTheKeys(), ans)
